...
def test_finds_md_files(vault_files: list[VaultFile]):
    assert len(vault_files) > 0


def test_all_files_are_md(vault_files: list[VaultFile]):
    assert all(f.path.suffix == ".md" for f in vault_files)


def test_all_files_have_mtime(vault_files: list[VaultFile]):
    assert all(f.mtime > 0 for f in vault_files)


def test_all_links_start_empty(vault_files: list[VaultFile]):
    assert all(f.links == [] for f in vault_files)


def test_all_files_exist(vault_files: list[VaultFile]):
...
def test_excludes_hidden_dirs(vault_files: list[VaultFile]):
    assert not any(
...
def test_bad_path_raises(bad_path: str):
